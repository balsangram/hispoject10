...
import {
  ...
  XCircle, CheckSquare, Ambulance, Navigation, Siren, Fuel, Gauge, UserCheck2,
  ZapOff, Flame, Rocket
} from 'lucide-react';

// BRAND COLORS: #7E22CE (Primary Purple), #FFFFFF (Cards/Workspace), #0F172A (Primary Text)
// TATKAL COLOR: #B45309 (Amber/Gold for Priority)

const MOCK_NOTIFICATIONS = [
  { id: 1, title: 'Stock Alert', msg: 'Paracetamol 500mg below threshold (450 units).', time: '10m ago', type: 'alert', icon: AlertTriangle },
  { id: 2, title: 'Tatkal Enrollment', msg: 'Priority ID #P10-T882 added to Gynae queue.', time: '15m ago', type: 'warning', icon: Flame },
...
export default function App() {
  ...
  return (
    <div className="flex h-screen bg-slate-50/50 overflow-hidden font-sans no-scrollbar selection:bg-purple-100 selection:text-purple-900 animate-in fade-in zoom-in-95 duration-1000">
      ...
      <aside className={`${sidebarOpen ? 'w-72' : 'w-24'} transition-all duration-500 bg-white flex flex-col z-50 shadow-[4px_0_24px_rgba(0,0,0,0.02)] border-r border-slate-100`}>
        <div className="p-6 flex items-center gap-4 border-b border-slate-50">
          <div className="bg-[#7E22CE] p-2.5 rounded-xl shadow-lg shadow-purple-900/20 group cursor-pointer hover:scale-105 transition-transform" onClick={() => { setActiveTab('dashboard'); setSelectedSegment(null); setSelectedDoctor(null); }}>
          ...
          {sidebarOpen && (
            <div className="animate-in fade-in slide-in-from-left-2 duration-500 cursor-pointer" onClick={() => { setActiveTab('dashboard'); setSelectedSegment(null); setSelectedDoctor(null); }}>
              <h1 className="text-xl font-black text-[#7E22CE] tracking-tighter leading-none uppercase">Project 10</h1>
              <p className="text-[9px] font-black text-slate-400 uppercase tracking-[3px] mt-1">HIS</p>
            </div>
          )}
        </div>
        ...
        <nav className="flex-1 overflow-y-auto py-6 no-scrollbar px-4 space-y-1.5">
          ...
          {MODULES.map((dept) => (
            <div key={dept.id} className="mb-0.5">
              <button onClick={() => { if (!sidebarOpen) { setActiveTab(dept.id); setSelectedSegment(null); } else setExpandedDepts(prev => prev.includes(dept.id) ? prev.filter(id => id !== dept.id) : [...prev, dept.id]); }} className={`w-full flex items-center justify-between px-4 py-3 rounded-xl transition-all duration-300 ${activeTab === dept.id ? 'bg-slate-50 text-[#7E22CE] font-bold' : 'hover:bg-slate-50/50 text-slate-500'}`}>
                <div className="flex items-center gap-3.5"><dept.icon size={18} className={activeTab === dept.id ? 'text-[#7E22CE]' : 'text-slate-400'} /><span className={`font-bold text-[12.5px] tracking-tight ${sidebarOpen ? 'block' : 'hidden'}`}>{dept.name}</span></div>
                ...
                <div className="mt-1 space-y-0.5 ml-5 pl-3 border-l border-slate-100 animate-in fade-in slide-in-from-top-1 duration-200">
                  {dept.subsets.map(sub => (
                    <div key={sub.id} className="py-0.5">
                      <button onClick={() => setExpandedSubsets(prev => prev.includes(sub.id) ? prev.filter(id => id !== sub.id) : [...prev, sub.id])} className="w-full flex items-center justify-between py-1.5 text-[9px] font-black text-slate-400 uppercase tracking-widest hover:text-[#7E22CE] transition-colors">{sub.name}</button>
                      {expandedSubsets.includes(sub.id) && (
                        <div className="space-y-0.5 mt-1 pb-1">
                          {sub.segments.map(seg => (
                            <button key={seg} onClick={() => { setSelectedSegment({ name: seg, parent: sub.name }); setActiveTab(dept.id); setSelectedDoctor(null); }} className={`w-full text-left py-1.5 px-3 text-[11px] font-bold rounded-lg transition-all flex items-center gap-2 group ${selectedSegment?.name === seg ? 'text-[#7E22CE] bg-purple-50' : 'text-slate-500 hover:text-[#7E22CE] hover:bg-slate-50'}`}>
                              <div className={`w-1 h-1 rounded-full transition-all ${selectedSegment?.name === seg ? 'bg-[#7E22CE]' : 'bg-slate-200 group-hover:bg-[#7E22CE]'}`}></div>{seg}
                            </button>
                          ))}
                        </div>
                      )}
        ...
        <div className="p-6 border-t border-slate-50">
          <div onClick={() => setSelectedSegment({ name: 'UserProfile', parent: 'Account' })} className="bg-slate-50 p-3.5 rounded-2xl border border-slate-100 flex items-center gap-3 shadow-sm hover:shadow-md transition-all cursor-pointer group hover:bg-white hover:border-[#7E22CE]/20">
            <div className="w-8 h-8 rounded-xl bg-[#7E22CE] flex items-center justify-center text-white font-black text-[10px]">AD</div>
            {sidebarOpen && (
              <div className="overflow-hidden">
                <p className="text-[11px] font-black text-slate-800 truncate uppercase tracking-tighter leading-none group-hover:text-[#7E22CE] transition-colors">Director</p>
      ...
      <main className="flex-1 flex flex-col overflow-hidden relative">
        <header className="h-16 bg-white border-b border-slate-100 flex items-center justify-between px-8 z-10 shadow-sm relative">
          <div className="flex items-center gap-6">
            <button onClick={() => setSidebarOpen(!sidebarOpen)} className="p-2.5 hover:bg-slate-50 rounded-xl text-[#7E22CE] transition-all"><Menu size={20} /></button>
            <div className="relative group">
              <Search className="absolute left-4 top-1/2 -translate-y-1/2 text-slate-400 transition-colors group-focus-within:text-[#7E22CE]" size={16} />
              <input type="text" placeholder="OmniSearch records..." className="pl-11 pr-6 py-2.5 bg-slate-50 border-none rounded-xl text-sm w-80 focus:ring-2 focus:ring-[#7E22CE]/5 outline-none transition-all placeholder:text-slate-400 font-medium text-slate-700" />
            </div>
          </div>
          <div className="flex items-center gap-8">
            <div className="flex flex-col items-end">
              <p className="text-[11px] font-black text-[#7E22CE] tracking-widest leading-none">{time.toLocaleTimeString('en-US', { hour: '2-digit', minute: '2-digit', second: '2-digit' })}</p>
            ...
            <div className="hidden lg:flex items-center gap-2 bg-purple-50 text-purple-700 px-3 py-1.5 rounded-2xl text-[9px] font-black border border-purple-100 tracking-[1.5px] uppercase"><div className="w-2.5 h-2.5 rounded-full bg-purple-500 animate-pulse ring-4 ring-purple-100"></div>LIVE SYNC</div>

            <div className="relative">
              <button onClick={() => setShowNotifications(!showNotifications)} className={`p-2.5 rounded-xl relative transition-all border shadow-sm ${showNotifications ? 'bg-[#7E22CE] text-white border-purple-800' : 'bg-white hover:bg-slate-50 text-slate-400 hover:text-[#7E22CE] border-slate-100'}`}>
                <Bell size={20} />
            ...
            <button onClick={() => setIsLoggedIn(false)} className="w-9 h-9 rounded-xl bg-[#7E22CE] flex items-center justify-center text-white font-black text-xs shadow-md hover:scale-105 transition-all">AD</button>
          </div>
        </header>
  ...
  function NavItem({ icon: Icon, label, active, onClick, collapsed }) {
    return (
      <button onClick={onClick} className={`w-full flex items-center gap-4 px-4 py-3.5 rounded-xl transition-all duration-300 relative group ${active ? 'bg-[#7E22CE] text-white shadow-xl shadow-purple-900/40' : 'text-slate-500 hover:bg-slate-50 hover:text-[#7E22CE]'}`}>
    ...
    <div className="p-10 max-w-screen-2xl mx-auto space-y-10 animate-in fade-in slide-in-from-bottom-4 duration-1000 no-scrollbar pb-40 relative">
      <div className="flex flex-col lg:flex-row justify-between items-start lg:items-end gap-6">
        <div className="space-y-3">
          <button onClick={onBack} className="text-[#7E22CE] font-black text-[11px] uppercase tracking-[4px] flex items-center gap-2 hover:translate-x-[-5px] transition-all"><ChevronRight size={16} className="rotate-180" /> Transport Hub</button>
          <h2 className="text-6xl font-black text-slate-900 tracking-tighter leading-none flex items-center gap-6"><div className="p-3.5 bg-[#7E22CE] text-white rounded-[1.5rem] shadow-2xl"><Ambulance size={36} /></div>Emergency <span className="text-[#7E22CE]">Fleet</span></h2>
        </div>

        <div className="flex items-center gap-4">
          <div className="relative group">
            <Search className="absolute left-5 top-1/2 -translate-y-1/2 text-slate-300 group-focus-within:text-[#7E22CE] transition-colors" size={18} />
            <input
              type="text"
              placeholder="Search Vehicle ID..."
              className="pl-14 pr-8 py-4 bg-white border border-slate-100 rounded-[2rem] text-sm w-80 shadow-2xl focus:ring-4 focus:ring-[#7E22CE]/5 outline-none font-bold placeholder:text-slate-200 text-slate-900"
            />
          </div>
          <button className="p-4 bg-[#7E22CE] text-white rounded-full shadow-lg shadow-purple-900/20 hover:scale-110 active:scale-95 transition-all">
            <RefreshCw size={20} />
          </button>
        </div>
      </div>

      {/* EXPANDED TRANSPORT KPIS */}
      <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 xl:grid-cols-6 gap-5">
        <ModernKPI label="Active Fleet" value="12/15" icon={Ambulance} trend="Operational" color="slate" />
        <ModernKPI label="Emergency" value="4" icon={Siren} trend="On Mission" color="rose" critical />
        <ModernKPI label="Avg Response" value="8.4m" icon={Timer} trend="-1.2m" color="slate" />
        <ModernKPI label="Fuel Readiness" value="78%" icon={Fuel} trend="Optimal" color="slate" />
        <ModernKPI label="Maintenance" value="2" icon={Wrench} trend="Service Due" color="rose" />
        <ModernKPI label="Drivers" value="14" icon={UserCheck2} trend="On-Duty" color="slate" />
      </div>
          ...
          <div key={amb.id} className="bg-white rounded-[2.5rem] border border-slate-100 p-2 shadow-xl shadow-slate-200/50 group relative overflow-hidden transition-all hover:-translate-y-1">
            <div className="p-6 pb-2 relative z-10">
              <div className="flex justify-between items-start mb-6">
                <div className="p-3 bg-slate-50 rounded-2xl text-[#7E22CE] border border-slate-100"><Ambulance size={24} /></div>
                <span className={`px-3 py-1.5 rounded-xl text-[9px] font-black uppercase tracking-widest border ${amb.status === 'Available' ? 'bg-purple-50 text-purple-600 border-purple-100' :
            ...
            <div className="p-2 mt-2 grid grid-cols-2 gap-2">
              ...
              <button
                ...
                className="py-3 bg-[#7E22CE] text-white rounded-2xl font-black text-[10px] uppercase tracking-widest flex items-center justify-center gap-2 shadow-lg hover:bg-purple-950 transition-all"
              >
      ...
      <div className="bg-white w-full max-w-4xl h-[80vh] rounded-[3rem] shadow-2xl overflow-hidden flex flex-col relative" onClick={e => e.stopPropagation()}>
        ...
        <div className="absolute top-0 left-0 right-0 p-6 z-20 flex justify-between items-start pointer-events-none">
          <div className="bg-white/90 backdrop-blur-md p-6 rounded-[2rem] shadow-xl border border-slate-100 pointer-events-auto">
            ...
            <p className="text-[10px] font-bold text-[#7E22CE] uppercase tracking-widest mt-1">Live Telemetry</p>
        ...
        <div className="flex-1 bg-slate-100 relative overflow-hidden">
          ...
          <svg className="absolute inset-0 w-full h-full pointer-events-none">
            <path d="M 200 500 Q 400 300 600 400 T 800 200" fill="none" stroke="#7E22CE" strokeWidth="6" strokeLinecap="round" strokeDasharray="10,10" className="animate-pulse" />
          </svg>

          {/* Ambulance Marker */}
          <div className="absolute top-1/3 left-1/2 -translate-x-1/2 -translate-y-1/2">
            <div className="relative">
              <div className="w-24 h-24 bg-[#7E22CE]/20 rounded-full animate-ping absolute inset-0"></div>
              <div className="w-24 h-24 bg-white rounded-full shadow-2xl flex items-center justify-center relative border-4 border-white z-10">
                <Ambulance size={40} className="text-[#7E22CE]" />
              </div>
        ...
        <div className="bg-white border-t border-slate-100 p-6 flex justify-between items-center">
          <div className="flex gap-8">
            ...
            <div>
              ...
              <p className={`text-xl font-black ${ambulance.status === 'On Mission' ? 'text-rose-500' : 'text-purple-500'}`}>{ambulance.status}</p>
            </div>
          </div>
  ...
  return (
    <>
      ...
      <div className="absolute top-16 right-0 z-[70] w-96 bg-[#1e293b] rounded-[2rem] shadow-[0_35px_60px_-15px_rgba(0,0,0,0.6)] border-[6px] border-white overflow-hidden animate-in fade-in slide-in-from-top-4 duration-300 origin-top-right">
        {/* Distinct Header */}
        <div className="p-6 bg-[#7E22CE] flex justify-between items-center relative overflow-hidden">
          ...
          <div className="relative z-10">
            ...
            <p className="text-[10px] font-bold text-purple-100/80 uppercase tracking-widest mt-1">System Feed</p>
          </div>
        ...
        <div className="max-h-[60vh] overflow-y-auto no-scrollbar p-3 space-y-2">
          {MOCK_NOTIFICATIONS.map((note) => {
            const styles = {
              ...
              success: 'bg-purple-500/10 border-purple-500/20 text-purple-200',
  ...
  return (
    <div className="min-h-screen bg-white flex overflow-hidden font-sans no-scrollbar">
      <div className="hidden lg:flex w-1/2 relative flex-col justify-between p-16 overflow-hidden border-r border-slate-100 bg-slate-50">
        ...
        <div className="relative z-10">
          <div className="flex items-center gap-4 mb-16 animate-in slide-in-from-left duration-700"><div className="bg-[#7E22CE] p-3 rounded-2xl shadow-xl shadow-purple-900/20"><Activity className="text-white w-8 h-8" /></div><div><h1 className="text-3xl font-black text-[#7E22CE] tracking-tighter leading-none uppercase">Project 10</h1><p className="text-[11px] font-black text-purple-400 uppercase tracking-[4px] mt-1">HIS</p></div></div>
          <div className="space-y-10 max-w-lg mt-24 animate-in slide-in-from-bottom duration-1000"><div className="space-y-2"><p className="text-[10px] font-black text-[#7E22CE] uppercase tracking-[6px] opacity-80">Established 2010</p><h2 className="text-5xl font-black text-slate-900 tracking-tighter leading-tight uppercase">A Legacy of <br />Healthcare <br /> Excellence</h2></div><p className="text-slate-600 text-2xl font-medium leading-relaxed italic border-l-4 border-[#7E22CE] font-sans pl-8 py-3 max-w-md bg-white/50 backdrop-blur-sm rounded-r-xl">"Every heartbeat, every hope—we're here to turn your dream of parenthood into a reality."</p></div>
        </div>
        <div className="relative z-10 flex gap-12 border-t border-slate-200/50 pt-12 animate-in fade-in delay-500 duration-1000"><div className="space-y-1"><p className="text-4xl font-black text-[#7E22CE] tracking-tighter leading-none">14+</p><p className="text-[10px] font-black text-slate-500 uppercase tracking-widest leading-tight">Years of <br />Commitment</p></div><div className="space-y-1 border-l border-slate-200/50 pl-12"><p className="text-4xl font-black text-slate-900 tracking-tighter uppercase leading-none text-[#7E22CE]">Leading</p><p className="text-[10px] font-black text-slate-500 uppercase tracking-widest leading-tight">IVF Facility <br />in India</p></div></div>
      </div>
      <div className="flex-1 flex flex-col items-center justify-center p-8 bg-white"><div className="w-full max-w-md space-y-12"><div className="text-center lg:text-left space-y-2"><h3 className="text-3xl font-black text-slate-900 tracking-tighter uppercase leading-none">Terminal Login</h3><p className="text-slate-400 text-sm font-bold uppercase tracking-widest opacity-60">Authentication Required</p></div><form onSubmit={onLogin} className="space-y-8"><div className="space-y-6"><div className="space-y-3"><label className="text-[10px] font-black text-slate-500 uppercase tracking-[4px] ml-4">Email ID</label><div className="relative group"><User className="absolute left-5 top-1/2 -translate-y-1/2 text-slate-300 group-focus-within:text-[#7E22CE] transition-colors" size={18} /><input required type="email" placeholder="admin.hq@project10.com" className="w-full pl-14 pr-6 py-5 bg-slate-50 border-2 border-transparent rounded-[2rem] text-sm font-bold focus:border-[#7E22CE]/10 focus:bg-white transition-all outline-none text-slate-800" /></div></div><div className="space-y-3"><div className="flex justify-between items-center px-4"><label className="text-[10px] font-black text-slate-500 uppercase tracking-[4px]">Password</label><button type="button" className="text-[9px] font-black text-[#7E22CE] uppercase tracking-widest hover:underline">Forgot Key?</button></div><div className="relative group"><Lock className="absolute left-5 top-1/2 -translate-y-1/2 text-slate-300 group-focus-within:text-[#7E22CE] transition-colors" size={18} /><input required type="password" placeholder="••••••••••••" className="w-full pl-14 pr-6 py-5 bg-slate-50 border-2 border-transparent rounded-[2rem] text-sm font-bold focus:border-[#7E22CE]/10 focus:bg-white transition-all outline-none text-slate-800" /></div></div></div><button type="submit" className="w-full py-6 bg-[#7E22CE] text-white rounded-[2.5rem] font-black text-xs uppercase tracking-[6px] shadow-2xl hover:bg-purple-950 transition-all flex items-center justify-center gap-4 hover:scale-[1.01] active:scale-[0.98]">Authorize Access <ArrowRight size={16} /></button></form></div></div>
    </div>
  );
}
...
function ApothecaryHubView({ onBack }) {
  ...
  return (
    <div className="p-10 max-w-7xl mx-auto space-y-10 animate-in fade-in slide-in-from-bottom-4 duration-1000 no-scrollbar pb-40">
      <div className="flex flex-col lg:flex-row justify-between items-start lg:items-end gap-6">
        <div className="space-y-3">
          <button onClick={onBack} className="text-[#7E22CE] font-black text-[11px] uppercase tracking-[4px] flex items-center gap-2 hover:translate-x-[-5px] transition-all"><ChevronRight size={16} className="rotate-180" /> Hub Core</button>
          <h2 className="text-6xl font-black text-slate-900 tracking-tighter leading-none flex items-center gap-6"><div className="p-3.5 bg-[#7E22CE] text-white rounded-[1.5rem] shadow-2xl"><Pill size={36} /></div>P10 <span className="text-[#7E22CE]">Apothecary</span></h2>
        </div>
      ...
      <div className="bg-white rounded-[2.5rem] border border-slate-100 shadow-xl shadow-slate-200/50 overflow-hidden">
        <div className="px-10 py-8 border-b border-slate-50 bg-slate-50/20 flex flex-col lg:flex-row justify-between items-center gap-6">
          <div className="flex items-center gap-4">
            <div className="w-12 h-12 bg-purple-50 rounded-2xl flex items-center justify-center text-[#7E22CE] border border-purple-100 shadow-sm"><Archive size={24} /></div>
            <div>
          ...
          <div className="flex gap-2 overflow-x-auto no-scrollbar max-w-full">
            {categories.map(cat => (
              <button
                ...
                className={`px-5 py-2.5 rounded-xl text-[10px] font-black uppercase tracking-widest whitespace-nowrap transition-all border ${selectedFilter === cat ? 'bg-[#7E22CE] text-white border-transparent shadow-lg shadow-purple-900/10' : 'bg-white text-slate-400 border-slate-100 hover:border-purple-200 hover:text-[#7E22CE]'}`}
              >
                {cat}
              </button>
            ))}
            ...
            <tbody className="divide-y divide-slate-50">
              {filteredStock.length > 0 ? (
                filteredStock.map((item) => (
                  <tr key={item.id} className="group hover:bg-purple-50/30 transition-all">
                    <td className="px-6 py-6">
                      <p className="text-slate-900 font-black text-base tracking-tight">{item.name}</p>
                      <p className="text-[10px] font-bold text-[#7E22CE] uppercase tracking-widest mt-1 opacity-70">{item.category}</p>
                    </td>
                    ...
                    <td className="text-center">
                      <div className="flex flex-col items-center">
                        ...
                        <span className={`px-2 py-0.5 rounded text-[8px] font-black uppercase tracking-tighter ${item.status === 'In Stock' ? 'bg-purple-50 text-purple-600' :
                    ...
                    <td className="text-right pr-12">
                      <button className="px-5 py-2.5 bg-white border border-slate-100 rounded-xl font-black text-[9px] uppercase tracking-widest text-slate-700 shadow-sm hover:bg-[#7E22CE] hover:text-white hover:border-transparent transition-all">Restock</button>
                    </td>
                  </tr>
                ))
              ) : (
...
function DoctorProfileView({ doctor, onBack }) {
  ...
  return (
    <div className="p-8 lg:p-12 max-w-7xl mx-auto space-y-10 animate-in fade-in slide-in-from-right-8 duration-700 pb-40 relative">
      <div className="flex items-center gap-4">
        <button onClick={onBack} className="p-3 bg-white border border-slate-100 text-[#7E22CE] rounded-2xl hover:bg-purple-50 transition-all shadow-sm">
      ...
      <div className="grid grid-cols-1 lg:grid-cols-3 gap-10 relative z-10">
        <div className="lg:col-span-1 space-y-8">
          <div className="bg-white rounded-[3.5rem] border border-slate-100 shadow-2xl overflow-hidden p-3">
            <div className="aspect-square rounded-[3rem] overflow-hidden border-8 border-purple-50 shadow-inner">
            ...
            <div className="p-8 text-center">
              ...
              <p className="text-[#7E22CE] font-bold text-sm uppercase tracking-widest bg-purple-50 rounded-xl py-2 mb-6">{doctor.role}</p>
          ...
          <div className="bg-[#7E22CE] p-8 rounded-[3rem] text-white shadow-xl shadow-purple-900/30 space-y-6">
            ...
            <button onClick={() => setActiveAction('book')} className="w-full py-5 bg-white text-[#7E22CE] rounded-3xl font-black text-xs uppercase tracking-[4px] shadow-2xl hover:scale-[1.02] transition-all">Book Appointment</button>
          </div>
        </div>
          ...
          <div className="bg-white p-12 rounded-[3.5rem] border border-slate-100 shadow-xl space-y-10">
            <div className="space-y-4">
              <div className="flex items-center gap-4 border-b border-slate-50 pb-4">
                <BadgeCheck size={20} className="text-[#7E22CE]" />
          ...
          <div className="bg-white p-10 rounded-[3.5rem] border border-slate-100 shadow-xl overflow-hidden relative">
            <div className="flex justify-between items-center mb-8">
              <h4 className="text-xs font-black text-slate-900 uppercase tracking-[4px] flex items-center gap-3">
                <Clock size={18} className="text-[#7E22CE]" /> Availability Ledger
              </h4>
              <span className="text-[9px] font-black text-purple-500 uppercase tracking-widest flex items-center gap-2">
                <div className="w-1.5 h-1.5 rounded-full bg-purple-500 animate-pulse"></div> Updated 2m ago
              </span>
            </div>
            <div className="grid grid-cols-4 lg:grid-cols-7 gap-4">
              {['Mon', 'Tue', 'Wed', 'Thu', 'Fri', 'Sat', 'Sun'].map((day, idx) => (
                <div key={day} className={`p-4 rounded-2xl text-center flex flex-col gap-2 transition-all ${idx < 5 ? 'bg-purple-50 border border-purple-100' : 'bg-slate-50 border border-slate-100 opacity-40'}`}>
                  <span className="text-[9px] font-black text-slate-400 uppercase">{day}</span>
                  <span className="text-[11px] font-black text-[#7E22CE]">{idx < 5 ? '9A-5P' : 'Closed'}</span>
                </div>
              ))}
            </div>
          </div>
    ...
    <div className={`fixed inset-0 z-50 flex items-center justify-center p-4 transition-all duration-300 ${isAnimating ? 'opacity-100 backdrop-blur-md bg-slate-900/40' : 'opacity-0'}`}>
      ...
      >
        <div className="px-10 py-8 border-b border-slate-50 flex justify-between items-center bg-slate-50/50">
          <div className="flex items-center gap-4">
            <div className={`p-3 rounded-2xl text-white shadow-lg shadow-purple-900/10 ${action === 'book' ? 'bg-[#7E22CE]' : action === 'consult' ? 'bg-[#A855F7]' : 'bg-slate-800'}`}>
...
function BookingFlow({ doctor, onClose }) {
  ...
  if (isConfirmed) {
    return (
      <div className="text-center py-10 animate-in zoom-in duration-500">
        <div className="w-24 h-24 bg-purple-50 rounded-full flex items-center justify-center mx-auto mb-6 text-purple-500 border-4 border-purple-100 shadow-xl">
          <Check size={40} strokeWidth={4} />
        </div>
        <h4 className="text-2xl font-black text-slate-900 tracking-tight mb-2">
          {isTatkal ? 'Tatkal Confirmed!' : 'Booking Confirmed!'}
        </h4>
        <p className="text-slate-500 text-sm font-medium">Your appointment ID is <span className="font-bold text-slate-900">#P10-{isTatkal ? 'T' : 'S'}9921</span></p>
        <div className="mt-8 bg-slate-50 p-6 rounded-3xl border border-slate-100 inline-block text-left min-w-[300px]">
          ...
          <div className="flex justify-between items-center mt-4 pt-4 border-t border-slate-200/50">
            ...
            <span className={`text-[10px] font-black uppercase px-2 py-1 rounded-md ${isTatkal ? 'bg-amber-100 text-amber-700' : 'bg-purple-100 text-purple-700'}`}>
              {isTatkal ? 'Express Priority' : 'Standard'}
            </span>
          </div>
        </div>
        <div className="mt-10">
          <button onClick={onClose} className="px-10 py-4 bg-[#7E22CE] text-white rounded-[2rem] font-black text-xs uppercase tracking-[4px] shadow-xl hover:bg-purple-950 transition-all">Done</button>
        </div>
      </div>
    );
  }
  ...
  return (
    <div className="space-y-10 animate-in slide-in-from-bottom-4 duration-500">
      <div className="space-y-4">
        <h4 className="text-xs font-black text-slate-400 uppercase tracking-[4px]">Step 1: Select Service Class</h4>
        <div className="grid grid-cols-2 gap-4">
          <button
            onClick={() => setIsTatkal(false)}
            className={`p-6 rounded-[2rem] border-2 transition-all text-left relative overflow-hidden group ${!isTatkal ? 'border-[#7E22CE] bg-purple-50/50' : 'border-slate-100 bg-white hover:border-slate-200'}`}
          >
            <div className={`p-3 rounded-xl mb-4 w-fit transition-colors ${!isTatkal ? 'bg-[#7E22CE] text-white' : 'bg-slate-100 text-slate-400 group-hover:bg-slate-200'}`}><ZapOff size={20} /></div>
            <h5 className="font-black text-slate-900 text-lg leading-tight">Standard</h5>
            <p className="text-[10px] font-bold text-slate-400 uppercase tracking-widest mt-1">Normal Processing</p>
          </button>
          <button
            onClick={() => setIsTatkal(true)}
            className={`p-6 rounded-[2rem] border-2 transition-all text-left relative overflow-hidden group ${isTatkal ? 'border-amber-500 bg-amber-50/50' : 'border-slate-100 bg-white hover:border-amber-200'}`}
          >
            <div className={`p-3 rounded-xl mb-4 w-fit transition-colors ${isTatkal ? 'bg-amber-600 text-white shadow-lg' : 'bg-slate-100 text-slate-400 group-hover:bg-amber-100 group-hover:text-amber-600'}`}><Rocket size={20} /></div>
            <h5 className="font-black text-slate-900 text-lg leading-tight">Tatkal (Express)</h5>
            <p className="text-[10px] font-bold text-amber-600 uppercase tracking-widest mt-1">Priority Intake</p>
            {isTatkal && <div className="absolute top-4 right-4 text-amber-600 font-black text-[10px]">+₹2,500</div>}
          </button>
        </div>
      </div>

      <div className="space-y-4">
        <h4 className="text-xs font-black text-slate-400 uppercase tracking-[4px]">Step 2: Select Date</h4>
        <div className="flex gap-4 overflow-x-auto pb-2 no-scrollbar">
          {dates.map((d) => (
            <button
              key={d.date}
              onClick={() => { setSelectedDate(d); setSelectedTime(null); }}
              className={`flex-1 min-w-[80px] p-4 rounded-3xl border transition-all flex flex-col items-center gap-1 ${selectedDate?.date === d.date ? (isTatkal ? 'bg-amber-600 text-white border-transparent shadow-xl shadow-amber-900/20' : 'bg-[#7E22CE] text-white border-transparent shadow-xl shadow-purple-900/20') : 'bg-slate-50 text-slate-400 border-slate-100 hover:border-slate-300 hover:bg-white'}`}
            >
      ...
      {selectedDate && (
        <div className="space-y-4 animate-in fade-in slide-in-from-bottom-2 duration-500">
          <h4 className="text-xs font-black text-slate-400 uppercase tracking-[4px]">Step 3: Available Slots</h4>
          <div className="grid grid-cols-3 sm:grid-cols-4 gap-3">
            {times.map((t) => (
              <button
                key={t}
                onClick={() => setSelectedTime(t)}
                className={`py-3 rounded-xl text-[11px] font-black uppercase tracking-wider transition-all border ${selectedTime === t ? (isTatkal ? 'bg-amber-600 text-white border-transparent shadow-md' : 'bg-[#7E22CE] text-white border-transparent shadow-md') : 'bg-white text-slate-600 border-slate-200 hover:border-slate-300'}`}
              >
                {t}
              </button>
            ))}
      ...
      <div className="pt-6 border-t border-slate-50 flex justify-end">
        <button
          ...
          className={`px-12 py-5 rounded-[2rem] font-black text-xs uppercase tracking-[6px] shadow-xl transition-all flex items-center gap-3 ${selectedTime ? (isTatkal ? 'bg-amber-600 text-white hover:bg-amber-700 hover:scale-[1.02]' : 'bg-[#7E22CE] text-white hover:bg-purple-950 hover:scale-[1.02]') : 'bg-slate-100 text-slate-300 cursor-not-allowed'}`}
        >
          Confirm {isTatkal ? 'Tatkal' : 'Standard'} <ArrowRight size={16} />
        </button>
      </div>
    </div>
  );
    ...
    <div className="space-y-8 animate-in slide-in-from-bottom-4 duration-500">
      <div className="bg-slate-50 p-6 rounded-3xl border border-slate-100 flex items-center gap-6">
        <div className="w-20 h-20 bg-slate-200 rounded-2xl flex items-center justify-center overflow-hidden relative">
          ...
          <div className="absolute bottom-2 right-2 w-3 h-3 bg-purple-500 rounded-full border-2 border-white"></div>
        </div>
        <div>
      ...
      <div className="space-y-4">
        <div className="flex justify-between items-center p-4 border border-slate-100 rounded-2xl">
          <div className="flex items-center gap-4">
            <div className="p-3 bg-purple-50 text-purple-600 rounded-xl"><Video size={20} /></div>
        ...
        <div className="flex justify-between items-center p-4 border border-slate-100 rounded-2xl">
          <div className="flex items-center gap-4">
            <div className="p-3 bg-purple-50 text-purple-600 rounded-xl"><Mic size={20} /></div>
        ...
        <div className="flex justify-between items-center p-4 border border-slate-100 rounded-2xl">
          <div className="flex items-center gap-4">
            <div className="p-3 bg-purple-50 text-purple-600 rounded-xl"><Wifi size={20} /></div>
            <span className="font-bold text-slate-700 text-sm">Connection</span>
          </div>
          <span className="text-[10px] font-black text-purple-500 uppercase tracking-widest">Excellent</span>
        </div>
      </div>
...
function ScheduleView({ doctor }) {
  ...
  return (
    <div className="space-y-6 animate-in slide-in-from-bottom-4 duration-500">
      <div className="bg-purple-50 p-6 rounded-3xl border border-purple-100 flex items-start gap-4 mb-6">
        <Info size={20} className="text-[#7E22CE] shrink-0 mt-0.5" />
        <p className="text-xs font-bold text-[#7E22CE] leading-relaxed">
      ...
      <div className="space-y-3">
        {days.map((day) => (
          <div key={day.name} className={`flex items-center justify-between p-4 rounded-2xl border ${day.status === 'Closed' ? 'bg-slate-50 border-slate-100 opacity-60' : 'bg-white border-slate-100'}`}>
            <div className="flex items-center gap-4">
              <div className={`w-2 h-2 rounded-full ${day.status === 'Open' ? 'bg-purple-500' : day.status === 'Closed' ? 'bg-rose-400' : 'bg-amber-400'}`}></div>
      ...
      <div className="flex justify-between items-center bg-slate-50 p-8 rounded-[2.5rem] border border-slate-100 shadow-sm">
        <div>
          <div className="flex items-center gap-3 mb-2">
            <div className="p-2 bg-[#7E22CE] rounded-lg text-white"><Icon size={18} /></div>
            <p className="text-[10px] font-black text-[#7E22CE] uppercase tracking-[4px]">{greeting}, Admin</p>
          </div>
          <h2 className="text-4xl font-black text-slate-900 tracking-tighter leading-none mb-3">Project 10 <span className="text-[#7E22CE]">Command Console</span></h2>
          <div className="flex items-center gap-4">
            <div className="flex items-center gap-2 bg-white px-3 py-1.5 rounded-xl border border-slate-100 shadow-sm"><Calendar size={12} className="text-[#7E22CE]" /><span className="text-[11px] font-bold text-slate-700 uppercase tracking-widest">{currentTime.toLocaleDateString('en-US', { day: 'numeric', month: 'long', year: 'numeric' })}</span></div>
            <div className="bg-white px-3 py-1.5 rounded-xl border border-slate-100 text-[11px] font-bold text-slate-700 uppercase tracking-widest flex items-center gap-2 shadow-sm"><Flame size={12} className="text-amber-600" /> Tatkal Quota: 4/20</div>
          </div>
        </div>
        <div className="flex gap-3">
          ...
          <button className="px-6 py-3 bg-[#7E22CE] text-white rounded-xl text-[10px] font-black shadow-lg shadow-purple-900/20 hover:bg-purple-950 transition-all uppercase tracking-widest" onClick={() => onSelectSegment({ name: 'New Patient Entry' })}>+ Enroll Identity</button>
        </div>
      </div>

      <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-5 gap-4">
        <ModernKPI label="Bookings" value="248" icon={Calendar} trend="+12" color="slate" />
        <ModernKPI label="Tatkal Express" value="18" icon={Flame} trend="Priority" color="amber" sub="High Revenue Units" />
        <ModernKPI label="Avg Wait (Std)" value="24m" icon={Timer} trend="-2m" color="slate" />
        <ModernKPI label="Avg Wait (TKL)" value="4m" icon={Rocket} trend="Target: 5m" color="slate" />
        <ModernKPI label="Revenue MIS" value="₹12.4L" icon={DollarSign} trend="+14%" color="slate" />
      </div>
      ...
      <div className="grid grid-cols-1 lg:grid-cols-3 gap-6">
        <div className="lg:col-span-2 bg-white rounded-[2rem] border border-slate-100 shadow-xl shadow-slate-200/50 overflow-hidden group">
          <div className="px-8 py-6 border-b border-slate-50 flex justify-between items-center bg-slate-50/30">
            <h3 className="font-black text-slate-900 flex items-center gap-3 uppercase tracking-wider text-[10px]"><Clock size={16} className="text-[#7E22CE]" /> Recent Logins</h3>
            <div className="flex gap-2 items-center"><div className="w-1.5 h-1.5 rounded-full bg-purple-500 animate-pulse"></div><span className="text-[9px] font-black text-slate-500 uppercase tracking-widest">System Live</span></div>
          </div>
          <div className="p-2">
            <table className="w-full">
              ...
              <tbody className="divide-y divide-slate-50">
                <PatientRow name="Sarah Jenkins" mrn="P10-T882" dept="Elite IVF" time="10:15 AM" status="Consulting" isTatkal />
                ...
                <PatientRow name="Vikas Khanna" mrn="P10-T901" dept="Cardiac Science" time="08:30 AM" status="Waiting" isTatkal />
              </tbody>
            </table>
          </div>
        </div>
        ...
        <div className="bg-white p-8 rounded-[2rem] border border-slate-100 shadow-xl shadow-slate-200/50 flex flex-col justify-between group">
          <div className="flex justify-between items-center mb-6 border-b border-slate-50 pb-4"><h3 className="font-black text-slate-900 flex items-center gap-3 uppercase tracking-wider text-[10px]"><PieChart size={16} className="text-[#7E22CE]" /> Service Split</h3><button className="text-[9px] font-black text-slate-400 hover:text-slate-900 uppercase tracking-widest transition-colors">By Type</button></div>
          <div className="flex-1 flex flex-col items-center justify-center space-y-10">
            <div className="relative w-40 h-40">
              <svg viewBox="0 0 36 36" className="w-full h-full transform -rotate-90">
                <circle cx="18" cy="18" r="15.915" fill="transparent" stroke="#f1f5f9" strokeWidth="3"></circle>
                <circle cx="18" cy="18" r="15.915" fill="transparent" stroke="#7E22CE" strokeWidth="4" strokeDasharray="80 20" strokeDashoffset="0"></circle>
                <circle cx="18" cy="18" r="15.915" fill="transparent" stroke="#B45309" strokeWidth="4" strokeDasharray="20 80" strokeDashoffset="-80"></circle>
              </svg>
              <div className="absolute inset-0 flex items-center justify-center flex-col"><p className="text-3xl font-black text-slate-900 tracking-tighter leading-none">20%</p><p className="text-[8px] font-black text-slate-400 uppercase tracking-[2px] mt-1">Tatkal</p></div>
            </div>
            <div className="w-full grid grid-cols-2 gap-3">
              <LegendItem label="Standard" percent="80%" />
              <LegendItem label="Tatkal" percent="20%" isTatkal />
            </div>
          </div>
        </div>
      </div>
      ...
      <div className="bg-white rounded-[2.5rem] border border-slate-100 shadow-xl shadow-slate-200/50 overflow-hidden group">
        <div className="px-10 py-8 border-b border-slate-50 flex justify-between items-center bg-slate-50/20">
          ...
          <div className="flex gap-4">
            ...
            <button className="text-[10px] font-black text-slate-900 border-b-2 border-[#7E22CE] pb-1 uppercase tracking-widest transition-colors">Tomorrow</button>
          </div>
        </div>
        <div className="p-8">
          <div className="grid grid-cols-1 md:grid-cols-2 xl:grid-cols-4 gap-6">
            <UpcomingAppCard name="John Arackal" mrn="P10-T9945" time="02:30 PM" dept="IVF Elite" dr="Dr. Sabine S" type="Tatkal" isTatkal />
            <UpcomingAppCard name="Meera Nair" mrn="P10-9948" time="03:15 PM" dept="Pediatrics" dr="Dr. Ajayakumar J" type="Vaccination" />
            <UpcomingAppCard name="Ananya S" mrn="P10-T9955" time="04:45 PM" dept="Gynaecology" dr="Dr. Anju Aravindan" type="Tatkal" isTatkal />
  ...
  return (
    <div className="p-10 max-w-7xl mx-auto space-y-8 animate-in fade-in duration-500 pb-32">
      <div className="flex justify-between items-end">
        <div>
          <button onClick={onBack} className="text-[#7E22CE] font-black text-[10px] uppercase tracking-[4px] flex items-center gap-2 hover:translate-x-[-5px] transition-all mb-3"><ChevronRight size={16} className="rotate-180" /> {segment.parent}</button>
          <h2 className="text-5xl font-black text-slate-900 tracking-tighter leading-none">{segment.name}</h2>
          <div className="flex items-center gap-3 mt-4">
            <span className="px-3 py-1 bg-purple-50 text-[#7E22CE] text-[10px] font-black uppercase tracking-widest rounded-lg border border-purple-100">Live Operation</span>
        ...
        <div className="flex gap-3">
          <button className="p-3 bg-white border border-slate-200 rounded-xl text-slate-400 hover:text-[#7E22CE] shadow-sm transition-all"><Settings size={20} /></button>
          <button className="p-3 bg-[#7E22CE] text-white rounded-xl shadow-lg shadow-purple-900/20 hover:bg-purple-950 transition-all"><RefreshCw size={20} /></button>
        </div>
      </div>
      <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-6">
      ...
      <div className="bg-white rounded-[2.5rem] border border-slate-100 shadow-xl shadow-slate-200/50 overflow-hidden">
        <div className="px-8 py-6 border-b border-slate-50 bg-slate-50/30 flex justify-between items-center">
          <h3 className="font-black text-slate-900 flex items-center gap-3 uppercase tracking-wider text-[11px]"><FileText size={16} className="text-[#7E22CE]" /> Recent {segment.name} Records</h3>
          <button className="text-[10px] font-black text-[#7E22CE] uppercase tracking-widest hover:underline">View All Archive</button>
        </div>
        <div className="p-2">
          <table className="w-full">
            <thead>
            ...
            <tbody className="divide-y divide-slate-50">
              {[1, 2, 3, 4, 5].map((i) => (
                <tr key={i} className="group hover:bg-slate-50 transition-all">
                  ...
                  <td className="px-6 py-4 text-center"><span className="px-3 py-1 bg-purple-50 text-purple-600 rounded-lg text-[9px] font-black uppercase tracking-widest border border-purple-100">Completed</span></td>
                  <td className="px-6 py-4 text-center text-[10px] font-bold text-slate-400">Oct {10 + i}, 2024</td>
                  <td className="px-6 py-4 text-right pr-8"><button className="text-[#7E22CE] hover:text-purple-800"><MoreHorizontal size={16} /></button></td>
                </tr>
              ))}
            </tbody>
          </table>
...
function UserProfileView({ onBack }) {
  return (
    <div className="p-10 max-w-4xl mx-auto space-y-8 animate-in fade-in slide-in-from-bottom-8 duration-700">
      <button onClick={onBack} className="text-[#7E22CE] font-black text-[10px] uppercase tracking-[4px] flex items-center gap-2 hover:translate-x-[-5px] transition-all mb-3"><ChevronRight size={16} className="rotate-180" /> Back</button>
      <div className="bg-white rounded-[3rem] border border-slate-100 shadow-2xl overflow-hidden relative">
        <div className="h-40 bg-gradient-to-r from-[#7E22CE] to-purple-900 relative"><div className="absolute -bottom-16 left-12 w-32 h-32 rounded-[2rem] bg-white p-2 shadow-xl"><div className="w-full h-full bg-slate-100 rounded-[1.5rem] flex items-center justify-center text-4xl font-black text-[#7E22CE]">AD</div></div></div>
        <div className="pt-20 pb-12 px-12">
          <div className="flex justify-between items-start mb-8">
            <div><h2 className="text-3xl font-black text-slate-900 tracking-tight">Admin Director</h2><p className="text-[#7E22CE] font-bold uppercase tracking-widest text-xs mt-1">Headquarters • Level 1 Access</p></div>
            <button className="px-6 py-3 bg-[#7E22CE] text-white rounded-xl font-black text-xs uppercase tracking-widest shadow-lg shadow-purple-900/20 hover:bg-purple-950 transition-all">Edit Profile</button>
          </div>
          <div className="grid grid-cols-1 md:grid-cols-2 gap-8">
            ...
            <div className="space-y-6">
              ...
              <div className="space-y-4">
                <div className="flex items-center justify-between p-4 bg-slate-50 rounded-2xl border border-slate-100"><div className="flex items-center gap-3"><ShieldCheck size={18} className="text-purple-500" /><span className="font-bold text-slate-700 text-xs uppercase tracking-wider">2FA Enabled</span></div><span className="text-[10px] font-black text-slate-400">Active</span></div>
                <div className="flex items-center justify-between p-4 bg-slate-50 rounded-2xl border border-slate-100"><div className="flex items-center gap-3"><Key size={18} className="text-[#7E22CE]" /><span className="font-bold text-slate-700 text-xs uppercase tracking-wider">Last Password Change</span></div><span className="text-[10px] font-black text-slate-400">30 Days ago</span></div>
              </div>
            </div>
          </div>
        </div>
    ...
    <div className="p-12 max-w-screen-2xl mx-auto space-y-12 animate-in fade-in duration-1000 no-scrollbar pb-40">
      <div className="flex flex-col lg:flex-row justify-between items-start lg:items-end gap-8">
        <div className="space-y-2">
          <button onClick={onBack} className="text-[#7E22CE] font-black text-[10px] uppercase tracking-[4px] flex items-center gap-2 hover:translate-x-[-5px] transition-all mb-4"><ChevronRight size={16} className="rotate-180" /> Operational Hub</button>
          <h2 className="text-6xl font-black text-slate-900 tracking-tighter leading-none flex items-center gap-6"><div className="p-3 bg-white text-[#7E22CE] rounded-2xl shadow-xl border border-slate-100"><Stethoscope size={32} /></div>Specialist <span className="text-[#7E22CE]">Directory</span></h2>
        </div>
        <div className="relative group z-20">
          <Search className="absolute left-6 top-1/2 -translate-y-1/2 text-slate-300 group-focus-within:text-[#7E22CE] transition-colors" size={20} />
          <input
            type="text"
            placeholder="Search Specialist..."
            value={searchTerm}
            onChange={(e) => setSearchTerm(e.target.value)}
            className="pl-16 pr-8 py-5 bg-white border border-slate-100 rounded-[2rem] text-sm w-full lg:w-[450px] shadow-2xl focus:ring-4 focus:ring-[#7E22CE]/5 outline-none font-bold placeholder:text-slate-300 text-slate-900 transition-all"
          />
        </div>
      </div>
      ...
      <div className="space-y-4">
        <div className="flex items-center gap-4 border-b border-slate-100 pb-2">
          <Filter size={14} className="text-[#7E22CE]" />
        ...
        <div className="flex flex-wrap gap-3 overflow-x-auto no-scrollbar pb-2">
          {departments.map((dept) => (
            <button
              ...
              className={`px-6 py-3 rounded-2xl text-[11px] font-black uppercase tracking-widest transition-all border whitespace-nowrap flex items-center gap-3 ${selectedDept === dept.name ? 'bg-[#7E22CE] text-white border-transparent shadow-lg shadow-purple-900/20' : 'bg-white text-slate-500 border-slate-100 hover:border-[#7E22CE]/30 shadow-sm'}`}
            >
              {dept.name}
          ...
          filteredDoctors.map((doc, idx) => (
            <div key={idx} className="bg-white rounded-[3rem] p-2 border border-slate-100 shadow-xl shadow-slate-200/50 hover:-translate-y-2 transition-all duration-500 group flex flex-col relative overflow-hidden">
              <div className="absolute top-8 right-8 z-20">
                <span className={`px-3 py-1 rounded-full text-[9px] font-black uppercase tracking-widest border ${doc.status === 'Available' ? 'bg-purple-50 text-purple-600 border-purple-100' : 'bg-slate-50 text-slate-400 border-slate-100'}`}>
                  {doc.status}
                </span>
              </div>
              ...
              <div className="p-8 pb-4 flex flex-col items-center text-center relative z-10">
                <div className="relative mb-6">
                  <div className="w-28 h-28 rounded-[3rem] overflow-hidden border-4 border-purple-50 shadow-xl group-hover:scale-105 transition-transform duration-500">
                    {doc.img ? (
                      <img src={doc.img} alt={doc.name} className="w-full h-full object-cover" />
                    ) : (
                      <div className="w-full h-full bg-gradient-to-br from-[#7E22CE] to-purple-900 flex items-center justify-center text-white font-black text-3xl">
                  ...
                  <div className="absolute -bottom-2 -right-1 w-10 h-10 bg-white rounded-full flex items-center justify-center border-4 border-white shadow-lg">
                    <div className="w-full h-full bg-purple-50 rounded-full flex items-center justify-center text-[#7E22CE]">
                ...
                <p className="text-[#7E22CE] font-bold text-[11px] uppercase tracking-widest mb-4 px-4 bg-purple-50/50 rounded-lg py-1">{doc.role}</p>
              ...
              <div className="mt-auto bg-slate-50/50 rounded-[2.5rem] p-3">
                <div className="flex justify-between items-center bg-white rounded-[2rem] p-4 border border-slate-100 shadow-sm">
                  ...
                  <div className="flex gap-2">
                    <button className="w-10 h-10 rounded-2xl bg-purple-50 text-[#7E22CE] flex items-center justify-center hover:bg-[#7E22CE] hover:text-white transition-all shadow-sm">
                      <Calendar size={18} />
                    </button>
                    <button
                      onClick={() => onSelectDoctor(doc)}
                      className="w-10 h-10 rounded-2xl bg-[#7E22CE] text-white flex items-center justify-center shadow-lg shadow-purple-900/20 hover:scale-110 active:scale-95 transition-all"
                    >
                      <ArrowRight size={18} />
                    </button>
                  </div>
  ...
  return (
    <div className="p-10 max-w-screen-2xl mx-auto space-y-12 animate-in fade-in duration-1000 no-scrollbar pb-40">
      <div className="flex justify-between items-center bg-white p-8 rounded-[3rem] border border-slate-100 shadow-2xl shadow-slate-200/50 sticky top-0 z-50">
        <div className="flex items-center gap-6">
          <button onClick={onBack} className="p-3 bg-purple-50 text-[#7E22CE] rounded-2xl hover:bg-[#7E22CE] hover:text-white transition-all border border-purple-100 shadow-sm"><ChevronRight size={18} className="rotate-180" /></button>
          <div>
            <h2 className="text-4xl font-black text-slate-900 tracking-tighter flex items-center gap-4 leading-none uppercase">Organizational <span className="text-[#7E22CE]">Matrix</span></h2>
            <p className="text-[10px] font-black text-slate-400 uppercase tracking-[3px] mt-1">Project 10 Operational Hierarchy</p>
          </div>
        </div>
        <div className="flex gap-3">
          <button className="px-6 py-3 bg-slate-50 border border-slate-200 rounded-xl text-[9px] font-black text-slate-400 uppercase tracking-widest hover:text-purple-600 transition-all">Print Map</button>
          <button className="px-6 py-3 bg-[#7E22CE] text-white rounded-xl text-[9px] font-black uppercase tracking-widest shadow-lg shadow-purple-900/20">Expand All</button>
        </div>
      </div>
...
function ModernOrgCard({ label, sub, icon: Icon, type, small }) {
  const styles = {
    ...
    leadership: 'bg-purple-50 border-purple-200 text-purple-900 ring-8 ring-purple-50/50',
    director: 'bg-white border-purple-100 shadow-xl text-slate-800',
  ...
  return (
    <div className={`mx-auto flex flex-col items-center group cursor-pointer relative z-10 ${small ? 'w-full' : 'w-72'}`}>
      <div className={`w-full p-6 rounded-[2.5rem] border text-center relative overflow-hidden transition-all duration-500 ${styles[type]} ${small ? 'p-4 rounded-[1.5rem]' : ''}`}>
        ...
        {!small && <div className="absolute top-4 right-4 flex gap-1"><div className="w-1.5 h-1.5 rounded-full bg-purple-500 animate-pulse"></div></div>}
      </div>
    </div>
  );
}
...
function ModernKPI({ label, value, icon: Icon, trend, sub, color, critical }) {
  ...
  return (
    <div className={`bg-white p-6 rounded-3xl border ${critical ? 'border-rose-200' : (color === 'amber' ? 'border-amber-200 shadow-amber-900/10' : 'border-slate-100')} shadow-xl shadow-slate-200/50 hover:-translate-y-1 transition-all duration-500 group relative overflow-hidden`}>
      <div className="absolute top-0 right-0 p-4 opacity-[0.03] group-hover:scale-110 transition-transform duration-700 text-slate-900"><Icon size={80} /></div>
      <div className="flex justify-between items-start mb-6 relative z-10">
        <div className={`p-2.5 rounded-xl border ${accentColors[color || 'slate']} shadow-sm`}><Icon size={18} /></div>
        <span className={`text-[10px] font-black ${critical ? 'text-rose-500' : (color === 'amber' ? 'text-amber-600' : 'text-purple-600')} uppercase tracking-widest`}>{trend}</span>
      </div>
      <div className="relative z-10">
        <p className="text-slate-400 text-[10px] font-black uppercase tracking-[3px] mb-1.5 leading-none">{label}</p>
        <h4 className={`text-3xl font-black ${critical ? 'text-rose-600' : (color === 'amber' ? 'text-amber-800' : 'text-slate-900')} tracking-tighter mb-1.5 leading-none`}>{value}</h4>
        <p className="text-[10px] font-bold text-slate-400 uppercase tracking-widest opacity-80">{sub || 'Telemetry Live'}</p>
      </div>
    </div>
  );
}

function PatientRow({ name, mrn, dept, status, time, isTatkal }) {
  return (
    <tr className="group hover:bg-slate-50 transition-all duration-300">
      <td className="px-6 py-5 font-black text-slate-900 text-[15px] tracking-tight">
        <div className="flex items-center gap-3">
          {name}
          {isTatkal && <span className="bg-amber-100 text-amber-700 text-[8px] font-black uppercase px-2 py-0.5 rounded shadow-sm border border-amber-200">TATKAL</span>}
        </div>
        <p className="text-[9px] text-slate-400 font-bold uppercase tracking-widest mt-1 opacity-70">{mrn}</p>
      </td>
      <td className="px-6 py-5 text-center">
        <span className={`text-[9px] font-black uppercase px-3 py-1.5 rounded-xl border ${isTatkal ? 'bg-amber-50 text-amber-600 border-amber-100' : 'bg-slate-50 text-slate-400 border-slate-100'}`}>
          {isTatkal ? 'Express' : 'Standard'}
        </span>
      </td>
      <td className="px-6 py-5 font-bold text-slate-400 text-[10px] text-center">
        <div className="flex items-center justify-center gap-2"><Timer size={10} className="text-[#7E22CE]/40" />{time}</div>
      </td>
      <td className="px-6 py-5 text-right pr-6">
        <span className="px-4 py-2 rounded-xl text-[8px] font-black uppercase border tracking-widest transition-all inline-block border-slate-100 bg-white text-slate-700 group-hover:bg-[#7E22CE] group-hover:text-white group-hover:border-transparent shadow-sm">
          {status}
        </span>
      </td>
    </tr>
...
function UpcomingAppCard({ name, mrn, time, dept, dr, type, isTatkal }) {
  return (
    <div className={`bg-white border ${isTatkal ? 'border-amber-200 shadow-amber-900/5' : 'border-slate-100'} p-6 rounded-[2rem] shadow-sm hover:shadow-xl hover:shadow-slate-200/50 hover:-translate-y-1 transition-all duration-300 relative group/app overflow-hidden`}>
      {isTatkal && <div className="absolute top-0 right-0 p-2 bg-amber-500 text-white rounded-bl-2xl"><Rocket size={12} /></div>}
      <div className="flex justify-between items-start mb-6">
        <div className={`px-3 py-1.5 ${isTatkal ? 'bg-amber-600' : 'bg-[#7E22CE]'} text-white text-[9px] font-black rounded-lg shadow-lg shadow-purple-900/20`}>{time}</div>
        <button className="text-slate-200 hover:text-slate-900 transition-colors"><MoreHorizontal size={14} /></button>
      </div>
      <div className="space-y-1 mb-6">
        <h4 className={`font-black text-base tracking-tight ${isTatkal ? 'text-amber-900' : 'text-slate-900'}`}>{name}</h4>
        <p className="text-[9px] font-black text-slate-400 uppercase tracking-widest">{mrn}</p>
      </div>
      <div className="space-y-3 pt-4 border-t border-slate-50">
        <div className="flex items-center gap-3">
          <div className={`w-1 h-1 rounded-full ${isTatkal ? 'bg-amber-600' : 'bg-[#7E22CE]'}`}></div>
          <p className="text-[10px] font-bold text-slate-500 uppercase tracking-widest">{dept}</p>
        </div>
        <div className="flex items-center gap-3"><Stethoscope size={12} className="text-slate-300" /><p className="text-[11px] font-black text-slate-700 tracking-tight">{dr}</p></div>
      </div>
      <div className="mt-6 flex justify-between items-center">
        <span className={`text-[8px] font-black uppercase tracking-widest px-2 py-1 rounded-md border ${isTatkal ? 'bg-amber-50 text-amber-600 border-amber-200' : 'bg-slate-50 text-slate-400 border-slate-100'}`}>
          {type}
        </span>
        <div className={`w-2 h-2 rounded-full ${isTatkal ? 'bg-amber-100' : 'bg-purple-500/20'} flex items-center justify-center`}>
          <div className={`w-1 h-1 rounded-full ${isTatkal ? 'bg-amber-500' : 'bg-purple-500'}`}></div>
        </div>
      </div>
    </div>
  );
...
function LegendItem({ label, percent, isTatkal }) {
  return (
    <div className="flex items-center justify-between bg-slate-50 p-2.5 rounded-xl border border-slate-100 hover:shadow-sm transition-all cursor-default">
      <div className="flex items-center gap-2">
        <div className={`w-1.5 h-1.5 rounded-full ${isTatkal ? 'bg-amber-600' : 'bg-[#7E22CE]'}`}></div>
...
function NewPatientForm({ onBack }) {
  return (
    <div className="p-8 max-w-5xl mx-auto animate-in fade-in slide-in-from-bottom-12 duration-1000 pb-32"><div className="flex justify-between items-center mb-10"><div className="space-y-2"><button onClick={onBack} className="text-[#7E22CE] font-black text-[10px] uppercase tracking-[4px] flex items-center gap-3 hover:translate-x-[-5px] transition-all mb-4"><ChevronRight size={20} className="rotate-180" /> Operational Hub</button><h2 className="text-5xl font-black text-slate-950 tracking-tighter leading-none flex items-center gap-6"><div className="p-3 bg-white text-[#7E22CE] rounded-2xl shadow-xl border border-slate-100 shadow-slate-200/50"><UserPlus size={32} /></div>Identity <span className="text-[#7E22CE]">Registration</span></h2></div></div><div className="bg-white rounded-[3.5rem] border border-slate-100 shadow-2xl shadow-slate-200/50 overflow-hidden relative"><div className="grid grid-cols-1 lg:grid-cols-4 relative z-10"><div className="p-12 border-r border-slate-50 bg-slate-50/30"><h3 className="text-[#7E22CE] font-black text-xs uppercase tracking-[4px] mb-8 flex items-center gap-3"><Info size={16} className="text-[#A855F7]" /> Instruction</h3><div className="space-y-8"><InstructionBlock step="01" title="Validate Profile" desc="Match biometric and identity data." /><InstructionBlock step="02" title="Contact Sync" desc="Confirm notification numbers." /></div></div><div className="lg:col-span-3 p-12 space-y-12"><div className="space-y-8"><div className="flex items-center gap-4 border-b border-slate-50 pb-4"><Fingerprint size={18} className="text-[#7E22CE]" /><h4 className="text-slate-400 font-black text-[10px] uppercase tracking-[4px]">Identity Matrix</h4></div><div className="grid grid-cols-1 md:grid-cols-2 gap-8"><FormInput label="Full Legal Name" placeholder="First Last" icon={User} /><FormInput label="Date of Birth" placeholder="DD / MM / YYYY" icon={Calendar} /><FormInput label="Gender" type="select" options={['Male', 'Female', 'Other']} icon={Activity} /><FormInput label="Blood Group" type="select" options={['A+', 'B+', 'O+', 'AB+']} icon={Droplets} /></div></div><div className="pt-8 border-t border-slate-50 flex justify-end items-center gap-10"><button className="px-12 py-5 bg-[#7E22CE] text-white rounded-[2rem] font-black text-xs uppercase tracking-[6px] shadow-2xl hover:bg-purple-950 transition-all flex items-center gap-4 group">Authorize Identity <ArrowRight size={18} className="group-hover:translate-x-2 transition-transform" /></button></div></div></div></div></div>
  );
}

function InstructionBlock({ step, title, desc }) {
  return (
    <div className="space-y-2"><div className="flex items-center gap-3"><span className="text-[10px] font-black text-white bg-[#7E22CE] w-6 h-6 rounded-md flex items-center justify-center">{step}</span><p className="text-[#7E22CE] font-black text-[10px] uppercase tracking-widest">{title}</p></div><p className="text-slate-400 text-[10px] font-medium leading-relaxed pl-9">{desc}</p></div>
  );
}

function FormInput({ label, placeholder, type = "text", options = [], icon: Icon }) {
  return (
    <div className="space-y-3"><label className="text-[10px] font-black text-slate-400 uppercase tracking-[3px] ml-1">{label}</label><div className="relative group">{Icon && <Icon className="absolute left-5 top-1/2 -translate-y-1/2 text-slate-300 group-focus-within:text-[#7E22CE] transition-colors" size={16} />}{type === "select" ? (<select className={`w-full ${Icon ? 'pl-14' : 'px-6'} pr-6 py-5 bg-white border border-slate-100 rounded-2xl text-sm font-black text-slate-900 focus:border-[#7E22CE] transition-all outline-none appearance-none`}>{options.map(o => <option key={o}>{o}</option>)}</select>) : (<input className={`w-full ${Icon ? 'pl-14' : 'px-6'} pr-6 py-5 bg-white border border-slate-100 rounded-2xl text-sm font-black text-slate-900 focus:border-[#7E22CE] transition-all outline-none placeholder:text-slate-200`} placeholder={placeholder} />)}{type === "select" && <ChevronDown className="absolute right-5 top-1/2 -translate-y-1/2 text-slate-300 pointer-events-none" size={16} />}</div></div>
  );
}

function LaboratoryDashboard({ segment, onBack }) {
  return (
    <div className="p-12 max-w-screen-2xl mx-auto space-y-12 animate-in fade-in duration-1000 pb-40 no-scrollbar"><div className="flex justify-between items-center bg-white p-10 rounded-[3rem] border border-slate-100 shadow-xl shadow-slate-200/50"><div><button onClick={onBack} className="text-[#7E22CE] font-black text-[11px] uppercase tracking-[6px] mb-3 flex items-center gap-2"><ChevronRight size={18} className="rotate-180" /> Hub</button><h2 className="text-4xl font-black text-slate-900 tracking-tighter flex items-center gap-6 leading-none"><div className="p-4 bg-white border border-slate-100 rounded-[2rem] text-[#7E22CE] shadow-2xl"><FlaskConical size={32} /></div>{segment.name} <span className="text-slate-300 font-light tracking-normal opacity-40">/ HUB-01</span></h2></div></div><div className="grid grid-cols-1 md:grid-cols-4 gap-8"><PremiumLabCard label="Units" value="1,248" sub="Daily Intake" icon={TestTube} /><PremiumLabCard label="Active" value="142" sub="Urgent Triage" icon={Clock} /><PremiumLabCard label="Auth" value="89" sub="Pending Verify" icon={ClipboardCheck} /><PremiumLabCard label="TAT" value="94%" sub="Global Optimal" icon={TrendingUp} /></div></div>
  );
}

function PremiumLabCard({ label, value, sub, icon: Icon }) {
  return (
    <div className="bg-white p-10 rounded-[3rem] border border-slate-100 shadow-xl shadow-slate-200/50 relative overflow-hidden group hover:scale-[1.02] transition-all duration-700"><div className="p-4 rounded-2xl inline-block mb-10 shadow-inner bg-slate-50 text-[#7E22CE] border border-slate-100 group-hover:rotate-6 transition-transform"><Icon size={24} /></div><p className="text-slate-400 text-[10px] font-black uppercase tracking-[6px] mb-2 leading-none">{label}</p><p className="text-5xl font-black text-slate-900 tracking-tighter mb-2 leading-none">{value}</p><p className="text-[10px] font-bold text-slate-400 uppercase tracking-widest">{sub}</p></div>
  );
}

function PlaceholderContent({ tab }) {
  return (
    <div className="flex flex-col items-center justify-center h-full text-slate-200"><div className="p-20 bg-white rounded-full border border-slate-100 mb-8 shadow-2xl relative"><ShieldCheck size={100} className="text-[#7E22CE] opacity-20" /></div><h3 className="text-3xl font-black text-slate-900 uppercase tracking-[12px] mb-2 opacity-80 leading-none">ACTIVE</h3><p className="text-slate-400 font-black uppercase tracking-[5px] text-[10px]">Authorization Verified: {tab}</p></div>
  );
}